...
class BubbleDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        boxes = []
        imgs = cv2.cvtColor(np.asarray(img), cv2.COLOR_RGB2BGR)
        for i in range(num_objs):
            # print('11111111111111111111111')
            # print('num_objs', num_objs)
            # print('masks[i]', masks[i])
            # print(np.where(masks[i]))
            ...
            thickness = 1
            lineType = 4
        #     cv2.rectangle(imgs, (xmin, ymin), (xmax, ymax), point_color, thickness, lineType)
        # cv2.imwrite('ttt/12345.png',imgs)

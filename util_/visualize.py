...
#
# def visualize_infer(img, result):
...
#         x1, y1, x2, y2 = box.mul(255).byte().cpu().numpy()
#         cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 1)
#         cv2.putText(img, str(labels[idx]), (x1, y1), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 12)
#         cv2.putText(img, str(scores[idx]), (x2, y1), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 12)
#         img = cv2.addWeighted(img, alpha, masks[idx, 0].mul(255).byte().cpu().numpy(), meta, gamma)
#     return img
...
def display_instances(image, boxes, masks, class_ids, class_names,
    ...
    for i in range(N):
        ...
        # Mask
        mask = masks[i,0].cpu().mul(255).detach().numpy()
    ...
    f = plt.gcf()  # 获取当前图像
    f.savefig(r'result\{}.png'.format('result'))
    f.clear()  # 释放内存
    # plt.show()
